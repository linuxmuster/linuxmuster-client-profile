...
_SETTINGS_FILE = "/home/samba/progs/linuxmuster-client/conf/sshplus.conf"
_USERCONFIG = os.getenv("HOME") + "/.sshplus"
...
def get_sshplusconfig(filename):
...
def build_menu():
    ...
    app_list = get_sshplusconfig(_SETTINGS_FILE)

    #Add sshmenu config items if any
    app_list2 = get_sshplusconfig(_USERCONFIG)
    if app_list2 != []:
...
if __name__ == "__main__":
    ind = appindicator.Indicator("sshplus", "dialog-information",
                                 appindicator.CATEGORY_APPLICATION_STATUS)
    ind.set_label(_USERNAME)
